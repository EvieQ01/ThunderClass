...
【更改记录】
    ...
    2020-06-17 由邱亦文增加注释
    2020-06-20 由邱亦文增加多选类 ,修改FromMyAnswer, 增加 ToMultiAnswerResult
*************************************************************************/
#ifndef MESSAGE_H
#define MESSAGE_H
...
【接口说明】
    ...
    静态成员函数 FromTimeData 产生学生注意力时长消息
    静态成员函数 FromMyAnswer 产生一个学生的答案
    静态成员函数 MakeAboutToDismissMsg()产生即将下课消息
    静态成员函数 MakeRequestVoiceMsg() 产生要求学生开麦消息
    静态成员函数 MakeVoiceStopMsg();产生停止随机点名消息
    静态成员函数 MakeScreenStopMsg();产生停止屏幕共享消息
    静态成员函数 MakeCollectAnswMsg();产生收题消息
    ...
    静态成员函数 ToTimeData 还原一个时长消息
    静态成员函数 ToAQuestion 还原一个题目消息
    静态成员函数 ToAnswerResult 还原一个答案消息
...
public:
    ...
    static Message MakeAboutToDismissMsg();
    static Message MakeRequestVoiceMsg();
    static Message MakeVoiceStopMsg();
    static Message MakeScreenStopMsg();
    static Message MakeCollectAnswMsg();
    ...
    static Message FromTimeData(const QTime& Timeall, const QTime& TimeFocused);
    static Message FromMyAnswer(const int result, const QTime& time);
    static Message FromMyAnswer(const int result[], const QTime& time);
    ...
    static string ToTimeData(const Message& Msg);
    //转化成题目（不在乎题目类型，主要用于显示文本即可，类型在MsgThread里判断就好）
    static Question ToAQuestion(const Message& Msg);
    //转化成答案（不同类的题，答案的形式也不一样）
    static void ToSingleAnswerResult(const Message& Msg, int* const dest_Result, char* const dest_Time);
    static void ToMultiAnswerResult(const Message& Msg, int* const dest_Result, char* const dest_Time);
...
private:
    ...
    //转发的audio序列
    queue<Message> m_toSendtoAllMsgs;
